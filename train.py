...
def train(D, G, D_optimizer, G_optimizer, D_loss, G_loss, data_loader, options):
  ...
  for epoch in range(epoch_count):
    ...
    if epoch % 50 == 0:
      torch.save(G.state_dict(), 'trained/meu_modelo_foda_g_state_dict_' + str(epoch) + '.pt')
      torch.save(D.state_dict(), 'trained/better_trained_d_state_dict_' + str(epoch) + '.pt')
...
def eval_generator(G, data_loader, options):
  G.eval()
  real_test, static_test = prepare_static_test(data_loader, options)
  visualize_progress(G, real_test, static_test)
...
def visualize_progress(G, real, static):
  show_grayscale_image(real[0].to(torch.device("cpu")))
  show_grayscale_image(static[0].to(torch.device("cpu")))
  generator_output = G(static.to(torch.device("cpu")))[0]
  show_grayscale_image(generator_output)
...
def prepare_static_test2(data_loader, options):
    real_test = None
    static_test = None
    glyph_size, glyphs_per_image, data_type = itemgetter('glyph_size', 'glyphs_per_image', 'data_type')(options)
    ...
    for data in data_loader:
        print(data)
        # Supondo que o dataset contenha os nomes dos arquivos em data['file_name']
        if data['file_name'][0] == "acknowtt.0.0":  # Verifique o nome do arquivo no batch
            real_test = reshape_real_data(data, glyph_size, glyphs_per_image)
            static_test = prepare_generator_input(data, glyph_size, glyphs_per_image).type(data_type)
            break  # Saia do loop assim que encontrar o arquivo desejado

    return real_test, static_test

"""
Fontes mais parecidas com 8 bits:
712_serif.0.0
acknowtt.0.0
aesymatt.0.0
binxchr.0.0
